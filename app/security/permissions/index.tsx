...
export default function PermissionsListPage() {
  ...
  const comparePermissions = (
    oldPerms: SecurityPermission[],
    newPerms: SecurityPermission[],
  ): boolean => {
  ...
  const loadCustomPermissions = useCallback(async () => {
    try {
      ...
      if (response && response.data) {
        ...
        const customPerms = allPerms.filter(
          (perm: SecurityPermission) => !perm.isSystem,
        );
        ...
        const hasChanges =
          ...
          comparePermissions(previousPerms, customPerms);
        ...
        if (hasChanges && previousPerms.length > 0) {
          ...
          console.log(
            "[loadCustomPermissions] Permisos personalizados actualizados (hubo cambios):",
            customPerms.length,
          );
        } else if (previousPerms.length === 0) {
          // Primera carga
          console.log(
            "[loadCustomPermissions] Permisos personalizados cargados inicialmente:",
            customPerms.length,
          );
        }
      }
      ...
      if (item.submenu) {
        for (const subItem of item.submenu) {
          if (subItem.route === route) {
            ...
            console.warn(
              `El menuItem del submenu con route "${route}" tiene un id inválido: "${subItem.id}". Debe ser un UUID.`,
            );
          }
        }
      }
          ...
          if (column.items) {
            for (const columnItem of column.items) {
              if (columnItem.route === route) {
                ...
                console.warn(
                  `El menuItem de la columna con route "${route}" tiene un id inválido: "${columnItem.id}". Debe ser un UUID.`,
                );
              }
            }
          }
    ...
    for (const change of changes) {
      ...
      for (const customPermKey of customPermKeys) {
        ...
        const customPerm = customPermissions.find(
          (p) => p.id === customPermKey || p.code === customPermKey,
        );
        ...
        const originalCustomPerm = rolePermissions.some((p: any) => {
          ...
          return (
            (permId === customPerm.id || permId === customPerm.code) &&
  ...
  const loadRolesByCompany = useCallback(
    async (companyId: string | undefined) => {
      ...
      try {
        ...
        if (
          searchParams.roleId &&
          companyRoles.find((r) => r.id === searchParams.roleId)
        ) {
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <CenteredModal
        ...
        subtitle={
          t.security?.permissions?.subtitle ||
          "Selecciona un permiso para editar o crea uno nuevo"
        }
        width="90%"
        height="60%"
      >
      ...
      {isCreateModalVisible && (
        <SideModal
          ...
          subtitle={
            t.security?.permissions?.createSubtitle ||
      ...
      {isEditModalVisible && selectedPermissionId && (
        <SideModal
          ...
          subtitle={
            t.security?.permissions?.editSubtitle ||
