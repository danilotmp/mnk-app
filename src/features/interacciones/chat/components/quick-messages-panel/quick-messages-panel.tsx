...
export const QuickMessagesPanel = React.memo(
  ({
    ...
    const [editingMessage, setEditingMessage] = useState<CatalogEntry | null>(
      null,
    );
    ...
    const filteredRecommendations = recommendations.filter(
      (rec) =>
        searchFilter === "" ||
        rec.message.toLowerCase().includes(searchFilter.toLowerCase()),
    );
    ...
    const generateNextCode = (): string => {
      ...
      const companyMessages = quickMessages.filter(
        (msg) => msg.companyId === companyId,
      );
    ...
    const handleCreateMessage = async () => {
      if (!newMessageText.trim() || !catalogId || !companyId) {
        alert.showError(
          "Error",
          "Faltan datos necesarios para crear el mensaje",
        );
        return;
      }
      ...
      try {
        ...
        alert.showSuccess(
          "Mensaje creado",
          "El mensaje rápido se ha creado correctamente",
        );
        setNewMessageText("");
    ...
    const handleEditMessage = async () => {
      if (!editingMessage || !newMessageText.trim()) {
        alert.showError(
          "Error",
          "Faltan datos necesarios para editar el mensaje",
        );
        return;
      }
      ...
      try {
        ...
        alert.showSuccess(
          "Mensaje actualizado",
          "El mensaje rápido se ha actualizado correctamente",
        );
        setNewMessageText("");
    ...
    const handleDeleteMessage = async () => {
      ...
      try {
        ...
        alert.showSuccess(
          "Mensaje eliminado",
          "El mensaje rápido se ha eliminado correctamente",
        );
        setNewMessageText("");
        setEditingMessage(null);
        setShowEditModal(false);
    ...
    return (
      <>
        ...
        >
          ...
          <View
            style={[
              quickMessagesPanelStyles.headerContainer,
            ...
            >
              {allMessages.length > 0 ? (
                allMessages.map((item) => {
                  ...
                  const canEdit =
                    isQuickMessage && item.entry.companyId !== null;
