...
import {
    CatalogService,
    catalogDetailsToInstructionTypeOptions,
    catalogDetailsToPaymentMethodOptions,
    catalogDetailsToSimpleOptions,
...
export function PaymentsLayer({
  ...
  const [selectedMethodType, setSelectedMethodType] =
    useState<PaymentMethodType | null>(null); // Tipo de método seleccionado en la UI
  const [selectedMethod, setSelectedMethod] = useState<PaymentMethod | null>(
    null,
  ); // Método existente de la BD
  ...
  const [editingAccountData, setEditingAccountData] = useState<
    ...
    >
  >({}); // Datos de edición de cuentas
  const [editingInstructionId, setEditingInstructionId] = useState<
    string | null
  >(null); // ID de la instrucción en edición
  const [editingInstructionData, setEditingInstructionData] = useState<
    Record<
      string,
      {
    ...
    >
  >({}); // Datos de edición
  const [editingPreviousInstructionType, setEditingPreviousInstructionType] =
    useState<Record<string, PaymentInstructionType | null>>({}); // Guarda el tipo anterior antes de seleccionar una cuenta en edición
  ...
  const [previousInstructionType, setPreviousInstructionType] =
    useState<PaymentInstructionType | null>(null); // Guarda el tipo anterior antes de seleccionar una cuenta
  ...
  useEffect(() => {
    if (selectedMethod) {
      ...
      setSelectedAccount((prevAccount) => {
        if (
          prevAccount &&
          !selectedMethod.accounts?.find((acc) => acc.id === prevAccount.id)
        ) {
  ...
  useEffect(() => {
    ...
    const generalInstructions = allInstructions.filter(
      (inst) => !inst.paymentAccountId,
    );
    ...
    const loadCatalogs = async () => {
      ...
      try {
        ...
        setPaymentMethodOptions(
          catalogDetailsToPaymentMethodOptions(paymentMethodsResponse.details),
        );
        setInstructionTypeOptions(
          catalogDetailsToInstructionTypeOptions(
            instructionTypesResponse.details,
          ),
        );
        const accountTypes = catalogDetailsToSimpleOptions(
          accountTypesResponse.details,
        );
  ...
  const handleToggleIsActive = async (
    ...
    setSaving(true);
    try {
      await CommercialService.updatePaymentMethod(method.id, {
        isActive: newValue,
      });
      alert.showSuccess(
      ...
      if (selectedMethod?.id === method.id) {
        const updatedMethods = await CommercialService.getPaymentMethods(
          company.id,
          true,
        ); // admin=true desde el wizard
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      const payload: PaymentAccountPayload = {
        ...
        additionalData: accountForm.accountType
      ...
      const defaultAccountType =
        accountTypeOptions.length > 0 ? accountTypeOptions[0].value : "";
      setAccountForm({
        name: "",
        provider: "",
      ...
      const updatedMethod = updatedMethods.find(
        (m) => m.method === selectedMethodType,
      );
      if (updatedMethod) {
        setSelectedMethod(updatedMethod);
      }
    ...
    try {
      ...
      if (updatedMethod) {
        ...
        if (payload.paymentAccountId) {
          const account = updatedMethod.accounts?.find(
            (acc) => acc.id === payload.paymentAccountId,
          );
          if (account) {
  ...
  const handleAccountClick = (account: PaymentAccount) => {
    ...
    const defaultAccountType =
      !existingAccountType && accountTypeOptions.length > 0
  ...
  const handleSaveAccount = async (accountId: string) => {
    ...
    try {
      const payload: Partial<PaymentAccountPayload> = {
        ...
        additionalData: formData.accountType
          ? { accountType: formData.accountType }
          : undefined,
        status: formData.status,
      };

      await CommercialService.updatePaymentAccount(
        selectedMethod.id,
        accountId,
        payload,
      );
  ...
  const handleDeleteAccount = async (accountId: string) => {
    ...
    try {
      ...
      const payload: Partial<PaymentAccountPayload> = {
        ...
        additionalData: (account.additionalData as any)?.accountType
  ...
  const handleSaveInstruction = async (instructionId: string) => {
    ...
    try {
      ...
      await CommercialService.updatePaymentInstruction(
        selectedMethod.id,
        instructionId,
        payload,
      );
            ...
            {paymentMethodOptions.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  {existingMethod &&
                    hasAccountsOrInstructions(existingMethod) && (
                      <View style={styles.optionCardHeader}>
                        <CustomSwitch
                          value={existingMethod.isActive}
                          onValueChange={(newValue) =>
                            handleToggleIsActive(existingMethod, newValue)
                          }
                          disabled={saving}
                        />
                      </View>
                              ...
                              {isEditing ? (
                                ...
                                <>
                                  <StatusBadge
                                    ...
                                    statusDescription={
                                      typeof account.statusDescription ===
                                        "string" &&
                                      account.statusDescription.trim() !== ""
                            ...
                            <View style={{ marginTop: 24 }}>
                              ...
                              <View style={styles.listContainer}>
                                {accountInstructions.map((instruction) => {
                                  ...
                                  const formData = isEditing
                                    ? editingInstructionData[instruction.id]
                                    : null;
                                  const currentStatus =
                                    formData?.status ?? instruction.status;
                                  const typeOption =
                                    instructionTypeOptions.find(
                                      (opt) =>
                                        opt.value ===
                                    ...
                                    >
                                      ...
                                      {isEditing ? (
                                        <>
                                          <View style={styles.guidelineHeader}>
                                            <View
                                              style={styles.guidelineTitleRow}
                                            >
                                              <View
                                                style={{
                                                  flex: 1,
                                                          ...
                                                          if (
                                                            ...
                                                            setEditingInstructionData(
                                                              (prev) => {
                                                                ...
                                                                if (
                                                                  currentFormData.paymentAccountId
                                                                ) {
                                                                  return {
                                                                    ...prev,
                                                                    [instruction.id]:
                                                            ...
                                                            setEditingPreviousInstructionType(
                                                              (prev) => {
                                                                const next = {
                                                                  ...prev,
                                                                };
                                          ...
                                          {accounts.length > 0 && (
                                            <>
                                              ...
                                              >
                                                Deja vacío para instrucción
                                                general, o selecciona una cuenta
                                                para instrucción específica
                                              </ThemedText>
                                              <View
                                              ...
                                              >
                                                <TouchableOpacity
                                                  ...
                                                  onPress={() => {
                                                    const restoredType =
                                                      editingPreviousInstructionType[
                                                        instruction.id
                                                      ] ||
                                                      (formData?.instructionType ||
                                                      instruction.instructionType ===
                                                      ...
                                                      if (
                                                        ...
                                                        setEditingPreviousInstructionType(
                                                          (prev) => ({
                                                            ...prev,
                                                            [instruction.id]:
                                                              currentFormData.instructionType,
                                                          }),
                                                        );
                                                      }
                                                      setEditingInstructionData(
                                        ...
                                        >
                                          ...
                                          <StatusBadge
                                            status={
                                              typeof instruction.status ===
                                              "number"
                                                ? instruction.status
                                                : RecordStatus.ACTIVE
                                            }
                                            statusDescription={
                                              typeof instruction.statusDescription ===
                                                "string" &&
                                              instruction.statusDescription.trim() !==
                                                ""
                      ...
                      >
                        ...
                        <View style={styles.accountTypeSelector}>
                          {accountTypeOptions.map((option, index) => {
                            ...
                            const isSelected =
                              currentType === option.value || shouldBeSelected;
                    ...
                    >
                      ...
                      >
                        <TextInput
                          ...
                          keyboardType={
                            selectedMethodType === "online" ? "url" : "numeric"
                          }
                          autoCapitalize={
                            selectedMethodType === "online"
                              ? "none"
            ...
            {displayedInstructions.length > 0 && (
              <View style={styles.listContainer}>
                {displayedInstructions.map((instruction) => {
                  ...
                  const typeOption = instructionTypeOptions.find(
                    (opt) => opt.value === instruction.instructionType,
                  );
                    ...
                    >
                        ...
                        >
                          {isEditing ? (
                                        ...
                                        style={[
                                          styles.radioOptionHorizontal,
                                          {
                                            ...
                                            opacity:
                                              ...
                                              currentType !== option.value
                                                ? 0.5
                                                : 1, // Hacer más transparente cuando no está seleccionada
                                          },
                                        ]}
                                        onPress={() => {
                            ...
                            <TouchableOpacity
                              style={{ flex: 1 }}
                              onPress={() =>
                                handleInstructionTitleClick(instruction)
                              }
                              activeOpacity={0.7}
                            >
                              <ThemedText
                          ...
                          <View style={styles.badgeActionsContainer}>
                            {!isEditing && (
                              <>
                                <StatusBadge
                                  status={
                                    typeof instruction.status === "number"
                                      ? instruction.status
                                      : RecordStatus.ACTIVE
                                  }
                                  statusDescription={
                      ...
                      {isEditing && accounts.length > 0 && (
                        <>
                          ...
                          >
                            Deja vacío para instrucción general, o selecciona
                            una cuenta para instrucción específica
                          </ThemedText>
                            ...
                            <TouchableOpacity
                              ...
                              onPress={() => {
                                ...
                                const restoredType =
                                  ...
                                  (formData?.instructionType ||
                                  instruction.instructionType === "warning"
                                    ? "warning"
                                    : "general");
                                setEditingInstructionData((prev) => ({
                                  ...prev,
                                  [instruction.id]: {
                                    ...(prev[instruction.id] || {
                                      ...
                                      status: instruction.status,
                                      paymentAccountId:
                                        instruction.paymentAccountId || null,
                                    }),
                                    paymentAccountId: "",
              ...
              <Card variant="outlined" style={styles.formCard}>
                          ...
                          <TouchableOpacity
                            ...
                            style={[
                              styles.radioOptionHorizontal,
                              {
                                ...
                                width: isMobile ? "100%" : undefined,
                                opacity:
                                  isAccountSpecific && !isSelected ? 0.5 : 1, // Hacer más transparente cuando no está seleccionada
                              },
                            ]}
                            onPress={() => {
                              // "Por Cuenta" no puede ser seleccionada directamente por el usuario
                          ...
                          >
                            <View
                              style={[
                                ...
                                {
                                  borderColor:
                                    instructionForm.instructionType ===
                                    option.value
                                      ? colors.primary
                                      : isDark
                                        ? colors.text
                ...
                {accounts.length > 0 && (
                  <>
                    ...
                    >
                      Deja vacío para instrucción general, o selecciona una
                      cuenta para instrucción específica
                    </ThemedText>
                    <View style={styles.selectContainer}>
                      <TouchableOpacity
                        style={[
                          styles.selectOption,
                          {
                            borderColor: !instructionForm.paymentAccountId
                              ? colors.primary
                              : colors.border,
                        ...
                        onPress={() => {
                          ...
                          setInstructionForm((prev) => {
                            const restoredType =
                              previousInstructionType ||
                              (prev.instructionType === "warning"
                                ? "warning"
                                : "general");
                            return {
                              ...prev,
                      ...
                      >
                        <ThemedText type="body2" style={{ color: colors.text }}>
                          {(previousInstructionType ||
                            instructionForm.instructionType) === "warning"
                            ? "Advertencia General"
                            : "Instrucción General"}
                        </ThemedText>
                          ...
                          onPress={() => {
                            ...
                            setInstructionForm((prev) => {
                              ...
                              if (prev.instructionType !== "account_specific") {
                                setPreviousInstructionType(
                                  prev.instructionType,
                                );
                              }
                              return {
                                ...prev,
                ...
                >
                  <TextInput
                    ...
                    onChangeText={(val) =>
                      setInstructionForm((prev) => ({ ...prev, message: val }))
                    }
                    multiline
                    numberOfLines={4}
                  />
