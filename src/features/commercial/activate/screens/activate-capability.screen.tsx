...
const AVAILABLE_CAPABILITIES = [
  {
    id: 'chat-ia',
    title: 'Chat con IA',
    description: 'Asistente inteligente que interactúa con tus clientes por WhatsApp',
...
export function ActivateCapabilityScreen() {
  ...
  const params = useLocalSearchParams<{ companyId?: string; capabilityId?: string; token?: string }>();

  const [loading, setLoading] = useState(false);
  const [activating, setActivating] = useState(false);
  const [companyId, setCompanyId] = useState(params.companyId || '');
  const [capabilityId, setCapabilityId] = useState(params.capabilityId || 'chat-ia');
  const [token, setToken] = useState(params.token || '');
  const [error, setError] = useState<{ message: string; detail?: string } | null>(null);
  const [success, setSuccess] = useState(false);

  const selectedCapability = AVAILABLE_CAPABILITIES.find(c => c.id === capabilityId);
        ...
        {success ? (
          <Card variant="elevated" style={styles.successCard}>
            <View style={styles.successContent}>
              ...
              <ThemedText type="h3" style={{ color: '#10b981', marginTop: 16, textAlign: 'center' }}>
                ¡Capacidad Activada!
              </ThemedText>
              <ThemedText type="body2" style={{ color: colors.textSecondary, marginTop: 8, textAlign: 'center' }}>
                La capacidad {selectedCapability?.title} ha sido activada correctamente para tu empresa.
                Serás redirigido a la página de capacidades...
              </ThemedText>
            </View>
          </Card>
        ) : (
          <>
                ...
                {AVAILABLE_CAPABILITIES.map((capability) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      {
                        borderColor: capabilityId === capability.id ? colors.primary : colors.border,
                        backgroundColor: capabilityId === capability.id ? colors.primary + '20' : colors.surface,
                      },
                    ]}
                  ...
                  >
                    <Ionicons
                      ...
                      size={32}
                      color={capabilityId === capability.id ? colors.primary : colors.textSecondary}
                    />
            ...
            <Card variant="elevated" style={styles.formCard}>
              ...
              >
                {activating && <ActivityIndicator size="small" color="#FFFFFF" style={{ marginRight: 8 }} />}
              </Button>
            </Card>
              ...
              <View style={styles.infoContent}>
                ...
                <View style={styles.infoText}>
                  ...
                  <ThemedText type="body2" style={{ color: colors.textSecondary }}>
                    Al activar una capacidad, se habilitará para tu empresa. Luego podrás configurarla 
                    desde la página de capacidades. Esta acción es reversible.
                  </ThemedText>
                </View>
              </View>
            </Card>
