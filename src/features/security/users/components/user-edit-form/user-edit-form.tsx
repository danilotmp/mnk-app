...
import {
    useBranchOptions,
    useCompanyOptions,
} from "@/src/domains/security/hooks";
import {
    PasswordInput,
    PhoneInput,
    StatusSelector,
...
export function UserEditForm({
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      try {
        ...
        if (
          Array.isArray(apiUserData?.companies) &&
          apiUserData.companies.length > 0
        ) {
          userCompanyIds = apiUserData.companies
            .map((c: any) => c.id)
            .filter((id: any) => id);
        ...
        if (
          ...
          Array.isArray((user as any).branches) &&
          (user as any).branches.length > 0
        ) {
          ...
          (user as any).branches.forEach((branchAccess: any) => {
            ...
            const branchCompanyId =
              branchAccess.branch?.companyId ||
              (userCompanyIds.length > 0 ? userCompanyIds[0] : null);
        ...
        const { message: errorMessage, detail: detailString } =
          extractErrorInfo(error, "Error al cargar usuario");
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const updatedUser = await UsersService.updateUserComplete(
        userId,
        updateData,
      );
  ...
  const loadingOptions =
    companiesLoading ||
    rolesLoading ||
    (formData.companyId ? branchesLoading : false);
