...
import { DynamicIcon, IconInput } from "@/src/domains/shared/components";
...
export function MenuAdminScreen() {
  ...
  const menuAdminTranslations = (t.security?.menuAdmin || {}) as NonNullable<
    NonNullable<Translations["security"]>["menuAdmin"]
  >;
  const { isMobile } = useResponsive();
  const alert = useAlert();
  const styles = useMemo(
    () =>
      createMenuAdminStyles(
        {
          colors,
          spacing,
          typography,
  ...
  const [originalMenuItems, setOriginalMenuItems] = useState<MenuAdminItem[]>(
    [],
  ...
  const [dragOverColumnParentId, setDragOverColumnParentId] = useState<
    string | null
  >(null);
  ...
  const loadMenuItems = useCallback(async () => {
    try {
      ...
      alert.showError(
        menuAdminTranslations.errorLoadingMenu || "Error al cargar el menú",
      );
    } finally {
      setLoading(false);
    }
  ...
  const convertToAdminItems = (
    ...
    return items
      .map((item, index) => {
        ...
        if (!item || !item.id) {
          console.warn(
            "Item inválido encontrado en convertToAdminItems:",
            item,
          );
          return null;
        }
  ...
  const startEdit = (item: MenuAdminItem) => {
    ...
    const statusValue =
      currentItem.status !== undefined && currentItem.status !== null
  ...
  const findOriginalUUID = (
    identifier: string,
    originalItems: MenuAdminItem[],
  ): string | null => {
  ...
  const getItemUUID = (item: MenuAdminItem): string | undefined => {
    ...
    console.warn(
      `No se encontró UUID para el item con id "${item.id}". Se tratará como item nuevo.`,
    );
    return undefined;
  };
  ...
  const saveSingleItem = async (itemId: string) => {
    ...
    try {
      ...
      const findItemAndParent = (
        items: MenuAdminItem[],
      ): { item: MenuAdminItem; parent: MenuAdminItem | null } | null => {
      ...
      const convertToMenuItem = (
        ...
        if (adminItem.isPublic !== undefined)
          menuItem.isPublic = adminItem.isPublic;
        ...
        if (parentItem) {
          ...
          if (parentItem.description)
            parentMenuItem.description = parentItem.description;
          if (parentItem.icon) parentMenuItem.icon = parentItem.icon;
          if (parentItem.isPublic !== undefined)
            parentMenuItem.isPublic = parentItem.isPublic;

          // Determinar si el item está en submenu o columns
          const isInSubmenu = parentItem.submenu?.some(
            (s) => s.id === adminItem.id,
          );
      ...
      if (result_sync.result.statusCode === 200) {
        ...
        if (result_sync.result.details?.validationErrors) {
          ...
          alert.showError(
            result_sync.result.description || "Error al guardar el item",
          );
        }
      }
    } catch (error: any) {
      ...
      alert.showError(
        error.message ||
          menuAdminTranslations.errorSavingMenu ||
  ...
  const updateItem = useCallback(
    (itemId: string, updates: Partial<MenuAdminItem>) => {
        ...
        return items.map((item) => {
          ...
          return {
            ...
            submenu: item.submenu
              ? updateItemsRecursive(item.submenu)
              : undefined,
      ...
      const updateItemIcon = (
        ...
        return items.map((item) => {
          ...
          if (item.submenu) {
            return {
              ...item,
              submenu: updateItemIcon(item.submenu, itemId, icon),
            };
          }
          if (item.columns) {
            return {
  ...
  const getModifiedItems = useCallback((): Set<string> => {
    ...
    const compareItems = (
      current: MenuAdminItem[],
      original: MenuAdminItem[],
    ): void => {
      ...
      current.forEach((item) => {
        ...
        const currentSubmenuIds = new Set(
          (item.submenu || []).map((sub) => sub.id),
        );
        const originalSubmenuIds = new Set(
          (originalItem?.submenu || []).map((sub) => sub.id),
        );
  ...
  const extractModifiedItems = useCallback(
    (items: MenuAdminItem[], modifiedIds: Set<string>): MenuAdminItem[] => {
      ...
      return items
        .map((item) => {
          ...
          if (item.submenu && item.submenu.length > 0) {
            const submenuResult = extractModifiedItems(
              item.submenu,
              modifiedIds,
            );
          ...
          if (item.columns && item.columns.length > 0) {
            const columnsResult = item.columns
              .map((col) => {
                const colItemsResult = extractModifiedItems(
                  col.items,
                  modifiedIds,
                );
      ...
      const convertToMenuItems = (items: MenuAdminItem[]): MenuItem[] => {
        ...
        items.forEach((item) => {
          ...
          if (
            ...
            if (
              convertedSubmenu &&
              Array.isArray(convertedSubmenu) &&
              convertedSubmenu.length > 0
            ) {
    ...
    const findParentAndAddColumn = (
      ...
      return items.map((item) => {
        ...
        return {
          ...item,
          submenu: item.submenu
            ? findParentAndAddColumn(item.submenu)
            : undefined,
          columns: item.columns
            ? item.columns.map((col) => ({
                ...col,
  ...
  const addItemToColumn = (parentId: string, columnId: string) => {
    ...
    const findParentAndAddToColumn = (
      items: MenuAdminItem[],
    ): MenuAdminItem[] => {
      return items.map((item) => {
        ...
        return {
          ...item,
          submenu: item.submenu
            ? findParentAndAddToColumn(item.submenu)
            : undefined,
          columns: item.columns
            ? item.columns.map((col) => ({
                ...col,
    ...
    setMenuItems((prev) => {
      ...
      if (newItemId) {
        ...
        setExpandedItems(
          (prevExpanded) => new Set([...prevExpanded, parentId]),
        );
  ...
  const updateColumnTitle = (
    ...
    const findParentAndUpdateColumn = (
      items: MenuAdminItem[],
    ): MenuAdminItem[] => {
      return items.map((item) => {
        if (item.id === parentId && item.columns) {
        ...
        return {
          ...item,
          submenu: item.submenu
            ? findParentAndUpdateColumn(item.submenu)
            : undefined,
          columns: item.columns
            ? item.columns.map((col) => ({
                ...col,
  ...
  const deleteColumnAndMoveItemsToSubmenu = (
    parentId: string,
    columnId: string,
  ) => {
    const findParentAndDeleteColumn = (
      items: MenuAdminItem[],
    ): MenuAdminItem[] => {
      return items.map((item) => {
        if (item.id === parentId && item.columns) {
          // Encontrar la columna a eliminar
          const columnToDelete = item.columns.find(
            (col) => col.id === columnId,
          );
        ...
        return {
          ...item,
          submenu: item.submenu
            ? findParentAndDeleteColumn(item.submenu)
            : undefined,
          columns: item.columns
            ? item.columns.map((col) => ({
                ...col,
  ...
  const findItemById = (
    items: MenuAdminItem[],
    itemId: string,
  ): { item: MenuAdminItem; parent: MenuAdminItem[]; index: number } | null => {
  ...
  const reorderColumns = useCallback(
    (parentId: string, draggedColumnId: string, targetColumnId: string) => {
      if (draggedColumnId === targetColumnId) return;

      setMenuItems((prev) => {
        const findParentAndReorderColumns = (
          items: MenuAdminItem[],
        ): MenuAdminItem[] => {
          return items.map((item) => {
            if (
              ...
              const draggedIndex = item.columns.findIndex(
                (col) => col.id === draggedColumnId,
              );
              const targetIndex = item.columns.findIndex(
            ...
            return {
              ...item,
              submenu: item.submenu
                ? findParentAndReorderColumns(item.submenu)
                : undefined,
              columns: item.columns
                ? item.columns.map((col) => ({
                    ...col,
                    items: col.items
                      ? findParentAndReorderColumns(col.items)
                      : [],
                  }))
                : undefined,
            };
  ...
  const findItemContext = (
    items: MenuAdminItem[],
    itemId: string,
    parent: MenuAdminItem | null = null,
    level: number = 0,
  ): {
    item: MenuAdminItem;
    parent: MenuAdminItem | null;
    parentItems: MenuAdminItem[];
    index: number;
    level: number;
    location: "root" | "submenu" | "column";
    columnId?: string;
  } | null => {
    for (let i = 0; i < items.length; i++) {
      if (items[i].id === itemId) {
  ...
  const removeItemFromLocation = (
    ...
    return items
      .map((item) => {
        ...
        if (
          item.columns &&
          item.columns.some((col) => col.items.some((ci) => ci.id === itemId))
        ) {
          return {
            ...item,
            columns: item.columns.map((col) => ({
        ...
        return {
          ...item,
          submenu: item.submenu
            ? removeItemFromLocation(item.submenu, itemId)
            : undefined,
          columns: item.columns
            ? item.columns.map((col) => ({
                ...col,
    ...
    return items.map((item) => {
      ...
      if (item.id === targetId) {
        const newItem = {
          ...
          level: item.level + 1,
          order:
            insertIndex !== undefined ? insertIndex : item.submenu?.length || 0,
        };
      ...
      if (item.submenu && item.submenu.some((sub) => sub.id === targetId)) {
        const targetIndex = item.submenu.findIndex(
          (sub) => sub.id === targetId,
        );
        const newItem = {
          ...draggedItem,
      ...
      return {
        ...item,
        submenu: item.submenu
          ? addItemToLocation(item.submenu, targetId, draggedItem, insertIndex)
          : undefined,
        columns: item.columns
          ? item.columns.map((col) => ({
              ...col,
              items: addItemToLocation(
                col.items,
                targetId,
                draggedItem,
                insertIndex,
  ...
  const reorderItems = useCallback((draggedId: string, targetId: string) => {
    ...
    setMenuItems((prev) => {
      ...
      if (
        draggedContext.parent?.id === targetContext.parent?.id &&
        ...
        if (
          draggedContext.location === "column" &&
          draggedContext.columnId &&
          draggedContext.parent
        ) {
          ...
          return prev.map((item) => {
            if (item.id === draggedContext.parent!.id) {
              return {
                ...item,
                submenu: findAndReorder(
                  item.submenu || [],
                  draggedContext.level,
                ),
              };
            }
            return item;
      ...
      if (
        targetContext.location === "column" &&
        targetContext.columnId &&
        targetContext.parent
      ) {
        // Agregar al final de la columna (o en la posición del target si se especifica)
        updated = addDraggedItemToColumn(
          updated,
          targetContext.parent.id,
          targetContext.columnId,
          draggedItem,
          targetContext.index,
        );
        ...
        const parentItem =
          prev.find((item) => item.id === targetContext.parent!.id) ||
          prev
            .flatMap((item) => item.submenu || [])
            .find((sub) => sub.id === targetContext.parent!.id) ||
          prev
            .flatMap((item) => item.columns?.flatMap((col) => col.items) || [])
            .find((ci) => ci.id === targetContext.parent!.id);
        ...
        if (parentItem && parentItem.columns && parentItem.columns.length > 0) {
          ...
          updated = addDraggedItemToColumn(
            updated,
            targetContext.parent.id,
            parentItem.columns[0].id,
            draggedItem,
          );
        } else {
          // Agregar como hermano del target (mismo nivel)
          updated = addItemToLocation(
            updated,
            targetContext.parent.id,
            draggedItem,
            targetContext.index,
        ...
        if (
          (targetItem.submenu && targetItem.submenu.length > 0) ||
          (targetItem.columns && targetItem.columns.length > 0)
        ) {
      ...
      const recalculateOrders = (
        ...
        return items.map((item, index) => ({
          ...
          level,
          submenu: item.submenu
            ? recalculateOrders(item.submenu, level + 1)
            : undefined,
          columns: item.columns
            ? item.columns.map((col) => ({
                ...col,
  ...
  const handleColumnMouseDown = useCallback(
    (e: any, parentId: string, columnId: string) => {
      ...
      >
        ...
        <View style={containerStyle}>
          ...
          {hasChildren && (
            <Tooltip
              text={
                isExpanded
                  ? menuAdminTranslations.collapse
                  : menuAdminTranslations.expand
              }
              position="top"
            >
              <TouchableOpacity
          ...
          <View style={[styles.itemRow, isMobile && styles.itemRowMobile]}>
            ...
            <TouchableOpacity
              ...
              style={[
                styles.itemNameButton,
                isMobile && styles.itemNameButtonMobile,
              ]}
            >
            ...
            <TouchableOpacity
              ...
              style={[
                styles.itemRouteButton,
                isMobile && styles.itemRouteButtonMobile,
              ]}
            >
              {item.route ? (
                <ThemedText
            ...
            <TouchableOpacity
              ...
              style={[
                ...
                isMobile && styles.itemStatusButtonMobile,
              ]}
            >
              <StatusBadge
                status={item.status}
                statusDescription={
                  item.status === 1
                    ...
                    : item.status === 0
                      ? menuAdminTranslations.inactive
                      : item.status === -1
        ...
        {isEditing && (
          ...
          >
            ...
            <View
              style={[styles.editFormRow, isMobile && styles.editFormRowMobile]}
            >
              <View
                style={[
                  styles.editFormField,
                  isMobile && styles.editFormFieldMobile,
                ]}
              >
                <ThemedText
                  type="body2"
                  style={[
                    styles.editFormLabel,
                    isMobile && styles.editFormLabelMobile,
                  ]}
                >
                  {menuAdminTranslations.icon}{" "}
                  <ThemedText style={styles.editFormLabelRequired}>
                    *
                  </ThemedText>
                </ThemedText>
                <View style={styles.iconInputContainer}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      try {
                        const iconsUrl =
                          APP_CONFIG.EXTERNAL_URLS.ICONS_DOCUMENTATION;
                        ...
                        alert.showError(
                          menuAdminTranslations.errorOpeningIconsDoc ||
                            "Error al abrir la documentación de iconos",
                        );
                      }
                    }}
                  >
            ...
            >
              <View
                style={[
                  styles.editFormFieldWide,
                  isMobile && styles.editFormFieldWideMobile,
                ]}
              >
                <ThemedText
                  type="body2"
              ...
              <View
                style={[
                  styles.editFormField,
                  isMobile && styles.editFormFieldMobile,
                  isMobile && styles.editFormFieldMobileWithTopMargin,
                ]}
              >
                <ThemedText
                  type="body2"
              ...
              {isMobile && (
                <View
                  style={[styles.editFormField, styles.editFormFieldMobile]}
                >
                  <View style={styles.publicToggleContainer}>
                    <CustomSwitch
                      value={formData.isPublic || false}
            ...
            <View style={styles.actionButtons}>
              ...
              <View style={styles.saveCancelActions}>
                ...
                <Button
                  title={
                    savingItem
                      ? menuAdminTranslations.saving
                      : menuAdminTranslations.save || t.common.save
                  }
              ...
              return (
                <View
                  ...
                  style={[
                    ...
                    isDragOverColumn ||
                    (draggingItemId && dragOverColumnId === column.id)
                ...
                >
                    ...
                    <Tooltip text={menuAdminTranslations.move} position="top">
                      <View
                        ...
                        {...(Platform.OS === "web"
                          ? {
                              onMouseDown: (e: any) =>
                                handleColumnMouseDown(e, item.id, column.id),
                            }
                          : {})}
                      >
                        <Ionicons
                    ...
                    {editingColumnId === column.id ? (
                      <View style={styles.columnTitleEditContainer}>
                        <TextInput
                          ...
                          placeholder={
                            menuAdminTranslations.columnTitlePlaceholder
                          }
                          placeholderTextColor={colors.textSecondary}
                          autoFocus
                          onBlur={() => {
                      ...
                      <>
                        ...
                        >
                          <ThemedText
                            type="subtitle"
                            style={styles.columnTitleButtonText}
                          >
                            {column.title}
                          </ThemedText>
                        </TouchableOpacity>
                        <Tooltip
                          text={menuAdminTranslations.deleteGrouping}
                          position="top"
                        >
                          <TouchableOpacity
                            onPress={() => {
                              ...
                              if (column.items && column.items.length > 0) {
                                alert.showConfirm(
                                  ...
                                  () => {
                                    deleteColumnAndMoveItemsToSubmenu(
                                      item.id,
                                      column.id,
                                    );
                                    alert.showSuccess(
                                      menuAdminTranslations.itemsMovedToSubmenu,
                                    );
                                  },
                                  () => {},
                                );
                  ...
                  {/* Items de la columna */}
                  {column.items.map((colItem, colIndex) =>
                    renderMenuItem(colItem, colIndex),
                  )}
                </View>
              );
            })}
      ...
      <View style={[styles.content, isMobile && styles.contentMobile]}>
        ...
        <View style={styles.pageHeader}>
          <View style={styles.pageHeaderTitle}>
            <View style={styles.pageHeaderRow}>
              ...
              >
                {menuAdminTranslations.title || "Administración del menú"}
              </ThemedText>
            </View>
            <ThemedText type="body1" style={styles.pageSubtitle}>
              {menuAdminTranslations.subtitle ||
                "Gestiona los items del menú del sistema"}
            </ThemedText>
          </View>
        </View>
        ...
        >
          <View style={styles.toolbarRow}>
            ...
            >
              ...
              <InputWithFocus
                containerStyle={[
                  ...
                  searchValue.length > 0
                    ? styles.searchInputContainerWithValue
            ...
            <Button title="" onPress={addRootItem} variant="primary" size="md">
          ...
          >
            {filteredItems.length === 0 ? (
              <View style={styles.emptyContainer}>
                ...
                >
                  {menuAdminTranslations.noItemsInMenu ||
        ...
        {hasUnsavedChanges && (
          ...
          >
            <ThemedText type="body2" style={{ color: colors.textSecondary }}>
              {getModifiedItems().size}{" "}
              {getModifiedItems().size === 1
                ? menuAdminTranslations.changePending
                : menuAdminTranslations.changesPending}{" "}
              {t.common.toSave || "de guardar"}
            </ThemedText>
