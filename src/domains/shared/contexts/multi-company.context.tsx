...
import { Branch, Company, MultiCompanyState, MultiCompanyUser } from "../types";
...
const MultiCompanyContext = createContext<MultiCompanyContextType | undefined>(
  undefined,
);
...
export function MultiCompanyProvider({ children }: MultiCompanyProviderProps) {
  ...
  const setUserContext = useCallback(
    async (user: MultiCompanyUser): Promise<void> => {
      try {
        setState((prev) => ({ ...prev, isLoading: true, error: null }));

        // Obtener currentCompanyId y currentBranchId de session storage
        const { UserSessionService } =
          await import("../services/user-session.service");
        const { UserContextService } =
          await import("../services/user-context.service");
        const userSessionService = UserSessionService.getInstance();
        const userContextService = UserContextService.getInstance();

        const currentCompanyId =
          (await userSessionService.getCurrentCompany()) ||
          user.companyIdDefault;
        const currentBranchId =
          (await userSessionService.getCurrentBranch()) ||
        ...
        if (currentCompany && user.companies && Array.isArray(user.companies)) {
          const foundCompanyInfo = user.companies.find(
        ...
        if (
          ...
          if (currentCompanyId) {
            const foundCompany = user.companies.find(
        ...
        if (currentBranch && branchesForCompany.length > 0) {
          finalCurrentBranch =
            branchesForCompany.find((b) => b.id === currentBranch.id) || null;
        }
        ...
        if (
          ...
          finalCurrentBranch =
            branchesForCompany.find((b) => b.id === currentBranchId) || null;
        }
        ...
        if (
          ...
          if (userResponse) {
            ...
            if (userResponse.branchIdDefault) {
              ...
              const defaultBranchInfo = branchInfos.find(
                (b) => b.id === userResponse.branchIdDefault,
              );
  ...
  const switchBranch = useCallback(
    async (newBranch: Branch): Promise<void> => {
      if (!state.user) {
        return;
      }
...
export function useMultiCompanyContext(): MultiCompanyContextType {
  ...
  if (context === undefined) {
    throw new Error(
      "useMultiCompanyContext debe ser usado dentro de un MultiCompanyProvider",
    );
  }
  return context;
}
