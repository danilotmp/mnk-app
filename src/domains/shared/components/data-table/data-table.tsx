...
export function DataTable<T = any>({
        ...
        <View style={styles.actionsContainer}>
          {orderedActions.map((actionOrConfig) => {
            ...
            if ("id" in actionOrConfig) {
              ...
              const isVisible =
                action.visible === undefined ? true : action.visible(item);
            ...
            const { type, action } = actionOrConfig as {
              type: "edit" | "delete";
              action: any;
            };
            const isVisible =
  ...
  const finalColumns = useMemo(() => {
      ...
      resultColumns = [
        {
          ...
          render: (item: T, index: number) => {
            ...
            if (editAction) {
              const isVisible =
                editAction.visible === undefined
                  ? true
                  : editAction.visible(item);
              if (isVisible) {
                canEdit = true;
            ...
            if (canEdit && editHandler) {
              return (
                <TouchableOpacity onPress={editHandler} activeOpacity={0.7}>
    ...
    const hasActions =
      (actions && actions.length > 0) || editAction || deleteAction;
    if (hasActions) {
      resultColumns.push({
        key: "actions",
  ...
  const getColumnMinWidth = (column: TableColumn<T>): number => {
    ...
    if (isCompactColumn(column)) {
      if (
        ...
        column.key === "telefono" ||
        column.key === "teléfono" ||
        column.label?.toLowerCase().includes("teléfono") ||
        column.label?.toLowerCase().includes("telefono")
      ) {
      ...
      if (
        ...
        column.key === "codigo" ||
        column.key === "código" ||
        column.label?.toLowerCase().includes("codigo") ||
        column.label?.toLowerCase().includes("código") ||
      ...
      if (
        ...
        column.key === "empresas" ||
        column.key === "company" ||
        column.key === "companies" ||
        column.label?.toLowerCase().includes("empresa") ||
        column.label?.toLowerCase().includes("empresas") ||
        column.label?.toLowerCase().includes("company") ||
        column.label?.toLowerCase().includes("companies")
      ) {
      ...
      if (
        column.key === "tipo" ||
        column.key === "type" ||
        column.label?.toLowerCase().includes("tipo") ||
        column.label?.toLowerCase().includes("type")
      ) {
  ...
  const renderPagination = () => {
    ...
    const filteredLimitOptions = limitOptions
      ? limitOptions.filter((option) => {
          ...
          const optionsGreaterThanTotal = limitOptions
    ...
    const finalLimitOptions =
      filteredLimitOptions.length > 0
