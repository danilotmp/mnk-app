...
export function MenuItemSelectorModal({
  ...
  const groupedItems = lastLevelItems.reduce((acc, item) => {
    const pathParts = item.path.split(' > ');
    const module = pathParts[0] || 'Otros';
  ...
  const filteredGroupedItems = Object.entries(groupedItems).reduce((acc, [module, items]) => {
    let filtered = items;
    
    // Si showOnlySelected está activo, mostrar solo los seleccionados
    if (showOnlySelected) {
      filtered = filtered.filter(item => selectedMenuItemIds.includes(item.id));
    }
  ...
  return (
    <CenteredModal
      visible={visible}
      onClose={onClose}
      title={t.security?.permissions?.selectMenuItems || 'Seleccionar Items del Menú'}
      subtitle={t.security?.permissions?.selectMenuItemsSubtitle || 'Selecciona los items del menú a los que se asociará este permiso'}
      width="90%"
      height="90%"
      footer={
              ...
              style={[
                {
                  ...
                  alignItems: 'center',
                  gap: 8,
                  backgroundColor: showOnlySelected ? colors.primary : colors.surface,
                  borderColor: showOnlySelected ? colors.primary : colors.border,
                }
              ]}
              activeOpacity={0.7}
            >
              <Ionicons 
                name={showOnlySelected ? "checkmark-circle" : "checkmark-circle-outline"} 
                size={20} 
              ...
              >
                {t.security?.permissions?.showOnlySelected || 'Solo seleccionados'}
              </ThemedText>
            </TouchableOpacity>
          </View>
        </View>
        ...
        {selectedMenuItemIds.length > 0 && (
          <View style={[styles.selectedCount, { backgroundColor: colors.primary + '20' }]}>
            <ThemedText type="body2" style={{ color: colors.primary, fontWeight: '600' }}>
              {selectedMenuItemIds.length} {selectedMenuItemIds.length === 1 ? 'item seleccionado' : 'items seleccionados'}
            </ThemedText>
          </View>
        )}
              ...
              return (
                <View key={moduleId} style={styles.moduleContainer}>
                  ...
                  >
                    ...
                    <View style={[styles.moduleBadge, { backgroundColor: 'transparent' }]}>
            ...
            {Object.keys(filteredGroupedItems).length === 0 && (
              <View style={styles.emptyContainer}>
                ...
                <ThemedText type="body2" variant="secondary" style={styles.emptyText}>
                  {searchValue.trim() ? 'No se encontraron items' : 'No hay items disponibles'}
                </ThemedText>
              </View>
            )}
          </ScrollView>
