...
export function PermissionActionIcons({
  ...
  const renderActionIcon = (action: {
    ...
    const hasPending = hasPendingChange
      ? hasPendingChange(route, action.name)
      : false;
    ...
    if (interactive && onTogglePermission) {
      return (
        <Tooltip key={action.name} text={action.tooltip} position="top">
          <TouchableOpacity
            onPress={() => onTogglePermission(route, action.name)}
          >
            {iconElement}
          </TouchableOpacity>
        </Tooltip>
  ...
  const renderCustomPermissionIcon = (perm: SecurityPermission) => {
    ...
    // Verificar si hay un cambio pendiente (no guardado aún)
    const hasPending = hasPendingChange
      ? hasPendingChange(route, permissionId)
      : false;
    ...
    if (interactive && onTogglePermission) {
      return (
        <Tooltip
          key={perm.id}
          text={perm.name || perm.code || ""}
          position="top"
        >
          <TouchableOpacity
            onPress={() => onTogglePermission(route, permissionId)}
          >
            {iconElement}
          </TouchableOpacity>
        </Tooltip>
